...
test.describe.serial('New Login Tests', () => {
    ...
    test('6. Verify password is shownHidden', async () => {
        ...
        await loginPage.verifyPasswordIsHidden(); // Убеждаемся, что по умолчанию пароль скрыт (type='password')
        await loginPage.togglePasswordVisibility(); // Кликаем на глазик
        await loginPage.verifyPasswordIsVisible(); // Убеждаемся, что пароль стал виден (type='text')
