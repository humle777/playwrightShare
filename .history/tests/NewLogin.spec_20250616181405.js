...
test.describe.serial('New Login Tests', () => {
    ...
    test('6. Verify password is shownHidden', async () => {
        ...
        await loginPage.verifyPasswordIsHiddenByValue(); // Часто это остается верным
        await loginPage.togglePasswordVisibility(); // Кликаем на глазик
        await loginPage.verifyPasswordIsVisibleByValue(testPassword); // Убеждаемся, что пароль стал виден (type='text')
        await loginPage.togglePasswordVisibility(); // Кликаем на глазик еще раз
        await loginPage.verifyPasswordIsHiddenByValue();
    });
