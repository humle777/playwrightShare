...
// const newValue = `${text} ${now.toISOString().slice(0, 19).replace('T', ' ')}`;
...
test.describe('Total', () => {
    ...
    test.describe('Sub_Total', () => {
        ...
        test('create_course', async ({ page, context }) => {
            ...
            /*
            await page.getByRole('button', { name: 'До списку тем' }).click();
            await page.locator('tr', { hasText: 'Тема 1' }).locator('a[title="Лекції теми"]').click();
            await page.getByRole('link', { name: ' Додати лекцію' }).click();
            await page.getByLabel('Назва лекції').click();
            await page.getByLabel('Назва лекції').fill('Лекція 1');


            await page.frameLocator('iframe[id="content_ifr"]').locator('body').fill('Ваш текст тут');



            await page.getByRole('button', { name: 'Зберегти' }).click();
            
            await expect(page.locator('.alert.alert-success')).toHaveText(/Лекція успішно створена!/);
            */
